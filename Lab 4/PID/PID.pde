...
while(1==1) {
    ...
    // Set position to follow circle
    float rotatePeriod = 3; // In seconds
    float radius = 0.5; // Max of 1
    if (followCircle) {
      counter = (counter+1)%(rotatePeriod*1000);
      xr = sin(counter/(rotatePeriod*1000)*2*PI)*radius;
      yr = cos(counter/(rotatePeriod*1000)*2*PI)*radius;
    }
